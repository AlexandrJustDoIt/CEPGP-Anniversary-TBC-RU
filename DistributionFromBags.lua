...
function CEPGP_DFB_print(str, err)
	...
	if err == nil then
		DEFAULT_CHAT_FRAME:AddMessage("|c00FFF569EPGP: " .. tostring(str) .. "|r");
	else
		DEFAULT_CHAT_FRAME:AddMessage("|c00FFF569EPGP:|r " .. "|c00FF0000Ошибка|r|c00FFF569 - " .. tostring(str) .. "|r");
	end
end
...
function CEPGP_DFB_CheckCepgpUiOverride(confirm)
	if CEPGP_DFB_Enabled and CEPGP_DFB_frame:IsShown() and confirm then
		...
		_G["CEPGP_distribute_roll_award"]:SetScript('OnEnter', function() 
			...
			GameTooltip:SetText("Вы не можете использовать \"Награду за высокий ролл\" в режиме из инвенторя.");
		end);
		_G["CEPGP_distribute_roll_award"]:SetScript('OnLeave', function() 
			GameTooltip:Hide();
		end);
	else  
		_G["CEPGP_distribute_roll_award"]:SetText("Награда за высокий ролл")
...
function CEPGP_DFB_SlashCmd(arg)
	if not CEPGP_DFB_frame:IsShown() then
		if CEPGP_DFB_Enabled then
			if C_PartyInfo.GetLootMethod() ~= 2 then
...
function CEPGP_DFB_LootFrame_Update(itemInfo)
	if C_PartyInfo.GetLootMethod() ~= 2 then
		CEPGP_DFB_print(L["Метод добычи - не Ответственный за добычу"], 1);
		return
	elseif CEPGP_isML() ~= 0 then
		CEPGP_DFB_print(L["Вы не являетесь Ответственным за добычу"], 1);
		return
	end
...
function CEPGP_DFB_ConfirmWinner(player)
	...
	if player == UnitName("player") then
		...
		_G["CEPGP_DFB_confirmation_desc"]:SetText(CEPGP_DFB_distItemLink .. "\n\n" .. player ..  L[" является победителем"])
		_G["CEPGP_DFB_confirmation_yes"]:SetText("Обмен")
	end
	CEPGP_DFB_confirmation:Show()
end
...
function CEPGP_DFB_error_open()
	...
	--player = normalizeName(player, true)
	...
	if isWinner then
		...
		if player == UnitName("player") then
			...
			if CheckInteractDistance(player, 2) then
				CEPGP_DFB_confirmation:Hide();
				--TargetUnit(player);
				InitiateTrade(player, true)
			else
				CEPGP_DFB_print(player.." ты слишком далеко", true)
			end
		end
	else
		CEPGP_DFB_DistPlayerBtn = nil
