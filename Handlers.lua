			...
			if not CEPGP.Loot.DelayResponses then
				...
				elseif not CEPGP_Info.Loot.ItemsTable[name] then
					...
					elseif not CEPGP.Loot.SuppressResponses then
						...
						if (CEPGP_getResponse(arg1) or CEPGP_getResponseIndex(arg1) or response < 5) then
							if CEPGP.Loot.RollAnnounce then
								CEPGP_sendChatMessage(name .. " (" .. class .. ") выбрал (" .. reason .. "). (Не член гильдии) (Ролл " .. roll .. ")", CEPGP.LootChannel);
							else
								CEPGP_sendChatMessage(name .. " (" .. class .. ") выбрал (" .. reason .. "). (Не член гильдии)", CEPGP.LootChannel);
							end
						end
					end
				end
						...
						if CEPGP_Info.Guild.Roster[player] then
							...
							if response then
								if response == "Highest Roll (Free)" then
									SendChatMessage(player .. " получает награду " .. itemName .. " бесплатно (самый высокий ролл)", CEPGP.Channel, CEPGP_Info.Language);
									CEPGP_addTraffic(player, CEPGP_Info.NormalizedPlayerName, response, EP, EP, GP, GP, id, tStamp);
								else
									SendChatMessage(player .. " получает награду " .. itemName .. " бесплатно", CEPGP.Channel, CEPGP_Info.Language);
									CEPGP_addTraffic(player, CEPGP_Info.NormalizedPlayerName, "Получает бесплатно", EP, EP, GP, GP, id, tStamp);
								end
							else
								SendChatMessage(player .. " получает награду " .. itemName .. " бесплатно", CEPGP.Channel, CEPGP_Info.Language);
								CEPGP_addTraffic(player, CEPGP_Info.NormalizedPlayerName, "Получает бесплатно", EP, EP, GP, GP, id, tStamp);
							...
							if index then
								SendChatMessage(player .. " получает награду " .. itemName .. " бесплатно (Список исключений)", CEPGP.Channel, CEPGP_Info.Language);
								CEPGP_addTraffic(player, CEPGP_Info.NormalizedPlayerName, "Получает бесплатно (Список исключений)", nil, nil, nil, nil, id, tStamp);
							end
						end
					end
					
				else
					SendChatMessage(itemName .. " был рапределен без EPGP", CEPGP.Channel, CEPGP_Info.Language);
					CEPGP_addTraffic(player, CEPGP_Info.NormalizedPlayerName, "Присуждается вручную", "", "", "", "", id, tStamp);
				end
			end;
			--[[if CEPGP_ntgetn(CEPGP_Info.Guild.Roster) < GetNumGuildMembers() and CEPGP_Info.Guild.Polling then
				table.insert(CEPGP_Info.RosterStack, callback);
