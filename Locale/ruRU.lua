local L = LibStub("AceLocale-3.0"):NewLocale("CEPGP", "ruRU");

if not L then return end
L["Molten Core / Onyxia"] = "Огненные недра / Ониксия"
L["Blackwing Lair"] = "Огненные недра / Ониксия"
L["Zul'Gurub"] = "Зул'Гуруб"
L["The Ruins of Ahn'Qiraj"] = "Руины Ан'Киража"
L["The Temple of Ahn'Qiraj"] = "Ан'Кираж"
L["Naxxramas"] = "Наксрамас"
L["World Encounters"] = "Мировые боссы"
L["Hyjal Summit"] = "Вершина Хиджала"
L["Onyxia"] = "Ониксия"
L["Lucifron"] = "Люцифрон"
L["Magmadar"] = "Магмадар"
L["Gehennas"] = "Гееннас"
L["Garr"] = "Гарр"
L["Baron Geddon"] = "Барон Геддон"
L["Shazzrah"] = "Шаззрах"
L["Sulfuron Harbinger"] = "Предвестник Сульфурон"
L["Golemagg the Incinerator"] = "Големагг Испепелитель"
L["Majordomo Executus"] = "Мажордом Экзекутус"
L["Ragnaros"] = "Рагнарос"
L["Flamewaker Elite"] = "Поджигатель-гвардеец"
L["Flamewaker Healer"] = "Поджигатель-целитель"
L["Razorgore the Untamed"] = "Бритвосмерт Неукротимый"
L["Vaelastrasz the Corrupt"] = "Валестраз Порочный"
L["Broodlord Lashlayer"] = "Предводитель драконов Разящий Бич"
L["Firemaw"] = "Огнечрев"
L["Ebonroc"] = "Черноскал"
L["Flamegor"] = "Пламегор"
L["Chromaggus"] = "Хроммагус"
L["Nefarian"] = "Нефариан"
L["High Priest Venoxis"] = "Верховный жрец Веноксис"
L["High Priestess Jeklik"] = "Верховная жрица Джеклик"
L["High Priestess Mar'li"] = "Верховная жрица Мар'ли"
L["High Priest Thekal"] = "Верховный жрец Текал"
L["High Priestess Arlokk"] = "Верховная жрица Арлокк"
L["Hakkar"] = "Хаккар"
L["Bloodlord Mandokir"] = "Мандокир Повелитель Крови"
L["Gri'lek"] = "Гри'лек"
L["Hazza'rah"] = "Хазза'рах"
L["Renataki"] = "Ренатаки"
L["Wushoolay"] = "Вушулай"
L["The Edge of Madness"] = "Край безумия"
L["Gahz'ranka"] = "Газ'ранка"
L["Jin'do the Hexxer"] = "Джин'до Проклинатель"
L["Kurinnaxx"] = "Куриннакс"
L["General Rajaxx"] = "Генерал Раджакс"
L["Moam"] = "Моам"
L["Buru the Gorger"] = "Буру Ненасытный"
L["Ayamiss the Hunter"] = "Аямисса Охотница"
L["Ossirian the Unscarred"] = "Оссириан Неуязвимый"
L["The Prophet Skeram"] = "Пророк Скерам"
L["Battleguard Sartura"] = "Боевой страж Сартура"
L["Fankriss the Unyielding"] = "Фанкрисс Непреклонный"
L["Princess Huhuran"] = "Принцесса Хухуран"
L["Emperor Vek'lor"] = "Император Век'лор"
L["Emperor Vek'nilash"] = "Император Век'нилаш"
L["The Twin Emperors"] = "Императоры-близнецы"
L["C'Thun"] = "К'Тун"
L["Princess Yauj"] = "Принцесса Яудж"
L["Vem"] = "Вем"
L["Lord Kri"] = "Лорд Кри"
L["The Bug Trio"] = "Трио Жуков"
L["Viscidus"] = "Нечистотон"
L["Ouro"] = "Оуро"
L["Anub'Rekhan"] = "Ануб'Рекан"
L["Grand Widow Faerlina"] = "Великая вдова Фарлина"
L["Maexxna"] = "Мексна"
L["Noth the Plaguebringer"] = "Нот Чумной"
L["Heigan the Unclean"] = "Хейган Нечестивый"
L["Loatheb"] = "Мерзот"
L["Instructor Razuvious"] = "Инструктор Разувиус"
L["Gothik the Harvester"] = "Готик Жнец"
L["Highlord Mograine"] = "Верховный лорд Могрейн"
L["Thane Korth'azz"] = "Тан Кортазз"
L["Lady Blaumeux"] = "Леди Бломе"
L["Sir Zeliek"] = "Сэр Зелиек"
L["The Four Horsemen"] = "Четыре всадника"
L["Patchwerk"] = "Лоскутик"
L["Grobbulus"] = "Гроббулус"
L["Gluth"] = "Глут"
L["Thaddius"] = "Таддиус"
L["Sapphiron"] = "Сапфирон"
L["Kel'Thuzad"] = "Кел'Тузад"
L["Lord Kazzak"] = "Владыка Каззак"
L["Azuregos"] = "Азурегос"
L["Emeriss"] = "Эмерисс"
L["Lethon"] = "Летон"
L["Ysondre"] = "Исондра"
L["Taerar"] = "Таэрар"
L["Karazhan"] = "Каражан"
L["Attumen the Huntsman"] = "Аттумен Охотник"
L["Moroes"] = "Мороуз"
L["Maiden of Virtue"] = "Благочестивая дева"
L["Opera Hall"] = "Оперный зал"
L["The Curator"] = "Смотритель"
L["Terestian Illhoof"] = "Терестиан Больное Копыто"
L["Shade of Aran"] = "Тень Арана"
L["Netherspite"] = "Пустогнев"
L["Chess Event"] = "Шахматы"
L["Prince Malchezaar"] = "Принц Малчезар"
L["Nightbane"] = "Ночная Погибель"
L["Gruul's Lair"] = "Логово Груула"
L["High King Maulgar"] = "Король Молгар"
L["Gruul the Dragonkiller"] = "Груул Драконобой"
L["Magtheridon's Lair"] = "Логово Магтеридона"
L["Magtheridon"] = "Магтеридон"
L["Serpentshrine Cavern"] = "Змеиное святилище"
L["Hydross the Unstable"] = "Гидросс Нестабильный"
L["The Lurker Below"] = "Скрытень из глубин"
L["Leotheras the Blind"] = "Леотерас Слепец"
L["Fathom-Lord Karathress"] = "Повелитель глубин Каратресс"
L["Morogrim Tidewalker"] = "Морогрим Волноступ"
L["Lady Vashj"] = "Леди Вайш"
L["The Eye"] = "Око"
L["Tempest Keep"] = "Крепость бурь"
L["Al'ar"] = "Ал'ар"
L["Void Reaver"] = "Страж Бездны"
L["High Astromancer Solarian"] = "Верховный звездочет Солариан"
L["Kael'thas Sunstrider"] = "Кель'тас Солнечный Скиталец"
L["Battle for Mount Hyjal"] = "Битва за гору Хиджал"
L["Rage Winterchill"] = "Лютый Хлад"
L["Anetheron"] = "Анетерон"
L["Kaz'rogal"] = "Каз'рогал"
L["Azgalor"] = "Азгалор"
L["Archimonde"] = "Архимонд"
L["Black Temple"] = "Черный храм"
L["High Warlord Naj'entus"] = "Верховный Полководец Надж'ентус"
L["Supremus"] = "Супремус"
L["Shade of Akama"] = "Тень Акамы"
L["Teron Gorefiend"] = "Терон Кровожад"
L["Gurtogg Bloodboil"] = "Гуртогг Кипящая Кровь"
L["Reliquary of Souls"] = "Воплощение Душ"
L["Mother Shahraz"] = "Матушка Шахраз"
L["The Illidari Council"] = "Совет Иллидари"
L["Illidan Stormrage"] = "Иллидан Ярость Бури"
L["Sunwell Plateau"] = "Плато Солнечного Колодца"
L["Kalecgos"] = "Калесгос"
L["Brutallus"] = "Бруталл"
L["Felmyst"] = "Пророк Скверны"
L["Eredar Twins"] = "Близнецы"
L["M'uru"] = "М'ару"
L["Kil'jaeden"] = "Кил'джеден"
L["Doom Lord Kazzak"] = "Владыка Судеб Каззак"
L["Doomwalker"] = "Судьболом"
L["Zul'Aman"] = "Зул'Аман"
L["Akil'zon"] = "Акил'зон"
L["Halazzi"] = "Халаззи"
L["Jan'alai"] = "Джан'алай"
L["Malacrass"] = "Малакрасс"
L["Nalorakk"] = "Налоракк"
L["Zul'jin"] = "Зул'джин"
L["Hex Lord Malacrass"] = "Повелитель проклятий Малакрасс"
L["%s have been defeated! %d EP has been awarded to the raid"] = "%s повержены! %d EP начислено всем участникам рейда"
L["%s has been defeated! %d EP has been awarded to the raid"] = "%s повержен(а)! %d EP начислено всем участникам рейда"
L["Updated auto award for "] = "Обновлена автоматическая награда для "
L["Updated EP award for "] = "Обновленная награда EP за "
L["Shoulder"] = "Плечи"
L["Instruction"] = "Удерживая клавишу Shift, перетащите предмет в инвентаре,\n чтобы начать процесс распределения добычи.\n\nВ конце вам нужно передать предмет победителю ВРУЧНУЮ."
L["The loot method is not Master Looter"] = "Метод распределения НЕ 'Отвественный за добычу'"
L["You are not the Loot Master"] = "Вы не являететь ответственным за распределение добычи."
L[" is the winner. Come to trade with me"] = " победитель. Приходите торговать со мной."
L[" is the winner"] = " победитель.\n\n.Нажмите кнопку [Торговля] с победителем."
L["Wrong Item"] = " не получен этот предмет."
L["Give GP without Trade confirm"] = "Дать GP без подтверждения торговли\n(Игрок может полуить GP, но не получить предмет)"
L["Head"] = "Голова"