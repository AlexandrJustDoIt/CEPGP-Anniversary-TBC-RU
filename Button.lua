...
function CEPGP_minThresholdDropdown(frame, level, menuList)
	...
	label:SetText("Минимальная редкость предмета для распределения по CEPGP")
	label:SetPoint("BOTTOMLEFT", frame, "TOPLEFT", 16, 3)
	label:SetJustifyH("LEFT")
	frame:HookScript("OnEnter", function(self)
        GameTooltip:SetOwner(self, "ANCHOR_TOPRIGHT")
        GameTooltip:SetText("Установите минимальную редкость предмета, чтобы окно добычи CEPGP автоматически отображалось в рейдах.\nТребуется, чтобы вы были главным сборщиком добычи и состояли в рейдовой группе.")
        GameTooltip:Show()
    end)
...
function CEPGP_defChannelDropdown(frame, level, menuList)
	...
	label:SetText("Канал для отчетов")
	label:SetPoint("BOTTOMLEFT", frame, "TOPLEFT", 16, 3)
	label:SetJustifyH("LEFT")
	frame:HookScript("OnEnter", function(self)
        GameTooltip:SetOwner(self, "ANCHOR_TOPRIGHT")
        GameTooltip:SetText("Устанавливает канал, на который отправляются изменения EPGP.")
        GameTooltip:Show()
    end)
    frame:HookScript("OnLeave", function(self)
        GameTooltip:Hide()
...
function CEPGP_lootChannelDropdown(frame, level, menuList)
	local label = frame:CreateFontString(nil, "OVERLAY", "CEPGPEditBoxOptionStringSmall")
	label:SetText("Канал отчетности об ответах")
	label:SetPoint("BOTTOMLEFT", frame, "TOPLEFT", 16, 3)
	label:SetJustifyH("LEFT")
	frame:HookScript("OnEnter", function(self)
        GameTooltip:SetOwner(self, "ANCHOR_TOPRIGHT")
        GameTooltip:SetText("Устанавливает канал, по которому отправляются ответы о добыче.\nСледующий пример: Игрок (класс) Нужно.")
        GameTooltip:Show()
    end)
    frame:HookScript("OnLeave", function(self)
        GameTooltip:Hide()
