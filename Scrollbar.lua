...
function CEPGP_UpdateTrafficScrollBar()
	...
	results = temp;
	CEPGP_traffic_display:SetText("Отображение записей: " .. CEPGP_Info.TrafficScope .. " - " .. math.min(CEPGP_Info.TrafficScope+499, #CEPGP.Traffic));
	...
	if #results > 0 then
		C_Timer.NewTicker(0.0001, function()
			...
			elseif results[i][10] and results[i][11] then
				...
				_G["TrafficButton" .. i .. "Share"]:SetText("Поделится");				
			end
...
function CEPGP_UpdateAttendanceScrollBar()
	...
	if #CEPGP.Attendance then
		_G["CEPGP_attendance_header_total"]:SetText("Всего записано снимков: " .. #CEPGP.Attendance);
	else
		_G["CEPGP_attendance_header_total"]:SetText("Всего записано снимков: 0");
	end
...
function CEPGP_UpdateLogScrollBar()
	...
	CEPGP_log_num:SetText("Показаны последние " .. #logs .. " messages");
end
